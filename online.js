// Piratwhist Online Multiplayer (v1.0.1.1)
...
// --- v1.0.1.1: dynamic round-table board (2–8 players) ---
...
function pcLayoutTunerActive(){
  // v1.0.1.1: Layout-tuner panelet må kun være synligt for spillernavn "LaBA".
...
function positionPlayBoard(n){
  ...
  if (isMobile){
    // v1.0.1.1 Dev + layout: SceneShift for mobile to utilize top space and
...
function runPlayAnimation(seat, cardObj, srcRect){
  ...
  fc.style.opacity = "1";
  const dur = 2000;
  const anim = flyArc(fc, dc.x, dc.y, { duration: dur, rotate: (seat === 0 ? -4 : 4), scale: v1.0.1.1 });
...
// v1.0.1.1 PC HUD sync + button wiring
function syncPcHud(){
...
// v1.0.1.1 no-fly zone: avoid overlap between hand area and the bottom-left opponent seat on PC
